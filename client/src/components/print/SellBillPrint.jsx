...
import { fetchSellBillPdf, sharePdf } from '../../lib/share'
import SellBillDocument from './SellBillDocument'
...
export default function SellBillPrint({ bill, onClose }) {
  ...
  const fileBaseName = bill?.billNumber || `sell-bill-${bill?.id || 'invoice'}`
  ...
  useEffect(() => {
    ...
    setPreparing(true)
    fetchSellBillPdf({ billId: bill.id, fileBaseName, signal: controller.signal })
  ...
  const handleShare = async () => {
    if (sharing) return
    setSharing(true)
    const toastId = 'sell-bill-share'
    try {
      let ready = pdf
      if (!ready) {
        toast.loading('Preparing PDF...', { id: toastId })
        ready = await fetchSellBillPdf({ billId: bill.id, fileBaseName })
        setPdf(ready)
      }
      const { method } = await sharePdf({
        pdf: ready,
        shareTitle: `Sell Bill ${bill?.billNumber || ''}`.trim(),
        shareText: `Sell bill ${bill?.billNumber || ''}`.trim(),
      })
