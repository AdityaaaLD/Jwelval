 ...
 * modal and the server-side PDF render route so the shared PDF is identical to
 * what the appraiser sees on screen.
 */
export default function SellBillDocument({ bill }) {
  const [profile, setProfile] = useState(null)
  useEffect(() => {
    let cancelled = false
    api.profile.get().then((p) => { if (!cancelled) setProfile(p) }).catch(() => {})
