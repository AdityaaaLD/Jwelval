...
export default function AppraiserProfile() {
  const [form, setForm] = useState({ appraiserName: '', businessName: '', mobile: '', email: '', upiId: '', address: '', logoPhoto: '' })
  useEffect(() => { api.profile.get().then((p) => setForm({ appraiserName: p.appraiser_name || '', businessName: p.business_name || '', mobile: p.mobile || '', email: p.email || '', upiId: p.upi_id || '', address: p.address || '', logoPhoto: p.logo_photo || '' })) }, [])
