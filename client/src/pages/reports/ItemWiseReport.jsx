...
export default function ItemWiseReport() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card grid gap-3 p-4 md:grid-cols-5 no-print"><input type="date" className="input" value={filters.date_from} onChange={(e) => setFilters({ ...filters, date_from: e.target.value })} /><input type="date" className="input" value={filters.date_to} onChange={(e) => setFilters({ ...filters, date_to: e.target.value })} /><input className="input" placeholder="Search item type" value={filters.description} onChange={(e) => setFilters({ ...filters, description: e.target.value })} /><select className="input" value={filters.format_type} onChange={(e) => setFilters({ ...filters, format_type: e.target.value })}>{['ALL', 'RUSHIKESH', 'DNYANESHWARI', 'BANK_OF_MAHA'].map((f) => <option key={f}>{f}</option>)}</select><button className="btn-primary" onClick={load}>Apply</button></div>
