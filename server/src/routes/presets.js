...
router.post('/banks', (req, res) => {
  const userId = req.user.id
  const { bankName, branch, branchCode, rateOfInterest, loanLtv, managerName, address, appIdPrefix, appIdDigits } = req.body
  const r = sqlite.prepare(`
    INSERT INTO bank_presets (bank_name, branch, branch_code, rate_of_interest, loan_ltv, manager_name, address, app_id_prefix, app_id_current_number, app_id_digits, user_id, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(bankName, branch || '', branchCode || '', Number(rateOfInterest) || null, Number(loanLtv) || null, managerName || '', address || '', appIdPrefix || '', 0, Number(appIdDigits) || 10, userId, new Date().toISOString())
...
router.put('/banks/:id', (req, res) => {
  ...
  const { bankName, branch, branchCode, rateOfInterest, loanLtv, managerName, address, appIdPrefix, appIdDigits } = req.body
  sqlite.prepare(`
    UPDATE bank_presets SET bank_name=?, branch=?, branch_code=?, rate_of_interest=?, loan_ltv=?, manager_name=?, address=?, app_id_prefix=?, app_id_digits=? WHERE id=? AND user_id=?
  `).run(bankName, branch || '', branchCode || '', Number(rateOfInterest) || null, Number(loanLtv) || null, managerName || '', address || '', appIdPrefix || '', Number(appIdDigits) || 10, id, userId)
