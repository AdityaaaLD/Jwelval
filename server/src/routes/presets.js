...
router.post('/banks', (req, res) => {
  const userId = req.user.id
  const { bankName, branch, branchCode, rateOfInterest, loanLtv, empanelmentId, managerName, bankLogo, address, appIdPrefix, appIdDigits, certificateRules } = req.body
  const r = sqlite.prepare(`
    INSERT INTO bank_presets (bank_name, branch, branch_code, rate_of_interest, loan_ltv, empanelment_id, manager_name, bank_logo, address, app_id_prefix, app_id_current_number, app_id_digits, certificate_rules, user_id, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(bankName, branch || '', branchCode || '', Number(rateOfInterest) || null, Number(loanLtv) || null, empanelmentId || '', managerName || '', bankLogo || '', address || '', appIdPrefix || '', 0, Number(appIdDigits) || 10, certificateRules || '', userId, new Date().toISOString())
...
router.put('/banks/:id', (req, res) => {
  ...
  const { bankName, branch, branchCode, rateOfInterest, loanLtv, empanelmentId, managerName, bankLogo, address, appIdPrefix, appIdDigits, certificateRules } = req.body
  sqlite.prepare(`
    UPDATE bank_presets SET bank_name=?, branch=?, branch_code=?, rate_of_interest=?, loan_ltv=?, empanelment_id=?, manager_name=?, bank_logo=?, address=?, app_id_prefix=?, app_id_digits=?, certificate_rules=? WHERE id=? AND user_id=?
  `).run(bankName, branch || '', branchCode || '', Number(rateOfInterest) || null, Number(loanLtv) || null, empanelmentId || '', managerName || '', bankLogo || '', address || '', appIdPrefix || '', Number(appIdDigits) || 10, certificateRules || '', id, userId)
